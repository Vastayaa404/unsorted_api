...
export const initialSystem = async () => {
  try {
    const token = jwt.sign({ prjName: 'Dora', destinationIp: '127.0.0.10', destinationPort: 5000 }, "1dmscnj823?/dsad_02sdawq-ds", { algorithm: 'HS512', expiresIn: '1m' });
    const status = await redis.get('Dora:State')
    const buildVersion = '0D06M12Y24B'; // Version/day/month/year/stable
    const startLoadTimestamp = new Date();
    const version = '0.0.1';

    try {
      redis.set('Dora:State', 'BFU');
      const resp = await axios.post('https://127.0.0.100:4430', { "request": "backend" }, { headers: { 'x-proxy-token': token } });
